...
class AgentGuard:
    ...
    def authorize(self, state, tool, arguments=None):
        ...
        if state not in self.policies:
            ...
            return {
                "allowed": False, "state": state, "tool": tool,
        ...
        for argument_name, rules in tool_rules.items():
            ...
            if "max" in rules:
                try:
                    ...
                    reason = (
                        ...
                        "for the configured maximum rule."
                    )
                    self.audit_log.record({
                        "state": state, "tool": tool,
                        "decision": "DENIED", "reason": reason
                ...
                if exceeds_max:
                    reason = (
                        f"Argument '{argument_name}' exceeds maximum allowed "
            ...
            if "min" in rules:
                try:
                    ...
                    reason = (
                        f"Argument '{argument_name}' has an invalid type "
                        "for the configured minimum rule."
                    )
                    self.audit_log.record({
                        "state": state, "tool": tool,
                        "decision": "DENIED", "reason": reason
                ...
                if below_min:
                    reason = (
                        f"Argument '{argument_name}' is below minimum allowed "
        ...
        return {
            "allowed": True, "state": state, "tool": tool,
            "reason": reason, "executed": False,
        }
    ...
    def execute_receipt(
        ...
        try:
            ...
            return {
                ...
                "error": str(error), "receipt_id": receipt.decision_id,
            }
        ...
        return {
            ...
            "result": result, "receipt_id": receipt.decision_id,
        }
